page 50032 "FAE Lines Subform"
{

    Caption = 'FAE Lines Subform';
    PageType = ListPart;
    ...
    layout
    {
        area(content)
        {
            repeater(General)
            {
                ...
                field("FAE Line Approval Status"; Rec."FAE Line Approval Status")
                {
                    ApplicationArea = All;
                    Editable = false;
                }
                ...
                field("Line Version Date"; Rec."Line Version Date")
                {
                    ApplicationArea = All;
                    Editable = false;
                }
        ...
        area(Processing)
        {
            action(SendApprovalRequest)
            {
                ...
                begin
                    ...
                    linewiseappsetup.SetFilter("On the Basis", '%1', linewiseappsetup."On the Basis"::"FAE Line Approval");
                    if linewiseappsetup.FindFirst() then begin
                        if linewiseappsetup.Enable = true then begin
                            // repeat
                            if Rec."FAE Line Approval Status" = Rec."FAE Line Approval Status"::Open then
                                Rec."FAE Line Approval Status" := Rec."FAE Line Approval Status"::"Pending For Approval";
                            Rec.Modify();
                            // until Rec.Next() = 0;
                        end;
                    end;
                ...
                begin
                    ...
                    if linewiseappsetup.FindFirst() then begin
                        if linewiseappsetup.Enable = true then begin
                            ...
                            if Rec."FAE Line Approval Status" = Rec."FAE Line Approval Status"::"Pending For Approval" then
                                Rec."FAE Line Approval Status" := Rec."FAE Line Approval Status"::Open;
                            Rec.Modify();
                        end;
                    end;
                end;
